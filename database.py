...
async def get_db() -> aiosqlite.Connection:
    ...
    db = await aiosqlite.connect(
        DB_FILE,
        timeout=30.0  # Таймаут ожидания блокировки (30 секунд)
    )
    db.row_factory = aiosqlite.Row  # Для доступа к колонкам по имени
    
    # Включаем WAL режим для поддержки одновременного доступа из нескольких процессов
    # WAL позволяет нескольким читателям и одному писателю работать одновременно
    await db.execute("PRAGMA journal_mode=WAL")
    # Оптимизируем настройки для многопроцессного доступа
    await db.execute("PRAGMA synchronous=NORMAL")  # Баланс между безопасностью и производительностью
